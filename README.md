...
- 추가 보정들
   - 최초 선수, 골대 객체 식별 및 팀 구분
      - 각 class별 탐지된 객체의 갯수가 부족한 경우
         - 다음 프레임으로 판단
      - 각 class별 탐지된 객체의 갯수가 필요 이상일 경우
         - 선수
               1. 경기장(+margin) 범위 밖의 객체 제외
               2. confidence score 기준 sort하여 총 인원 만큼만 slicing
               3. slicing된 선수 list를 다시 x좌표 기준으로 sort하여 왼쪽 선수들(5명 또는 6명)을 team_A로, 오른쪽 선수들을 team_B로 정의
         - 골대
               1. confidence score 기준 sort하여 상위 2개 객체만 slicing
               2. 2개중 x좌표값이 작은 객체를 team_A_goal_post로, x좌표값이 큰 객체를 team_B_goal_post로 정의
   - 공 보정
      - 경기장 (+margin) 범위 밖의 객체 제외
      - 경기장 내에서 탐지된 공 객체중 confidence score가 가장 높은 하나의 객체만 적용
   - 공통 보정
      - re-detected object
         - 잃어버린 객체를 다시 찾았을 때 해당 class의 잃어버렸던 객체의 id 부여
         - 잃어버린 객체를 다시 찾았을 때 해당 객체의 잃어버린 중간 프레임들의 위치값 보간
