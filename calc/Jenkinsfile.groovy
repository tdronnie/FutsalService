pipeline {
    ...
    stages {
        ...
        stage('Delete Previous calc Docker Container'){
            steps {
                script {
                    def  calcContainerExists = sh(script: "docker ps --filter=name=${CONTAINER_NAME}", returnStdout: true).trim()
                    if (calcContainerExists) {
