  ...
  return (
    <div className="flex items-center justify-center min-h-[100vh]">
      <div className="flex flex-col items-center">
        ...
        />
        <div className="m-1" onClick={() => handleNavigate({ path: "/login" })}>
          <GlobalButton isdisabled label="로그인" width="w-80" hover={false} />
        </div>
        <div
          className="m-1"
          onClick={() => handleNavigate({ path: "/signup" })}
        >
          <ReverseButton isdisabled label="회원가입" width="w-80" hover={false} />
        </div>
      </div>
    </div>
  );
